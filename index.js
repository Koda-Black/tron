...
const tronWeb = new TronWeb({
    ...
    privateKey: 'ceef8af1967a9e630a17e6b95fd4318b57530ff6333f47f0b8d4fa85c6138384' //mainnet    
...
async function main(fromAddress,addr,amt,pkey) {
    ...
    let {
        ...
        tronWeb.address.toHex(CONTRACT), 'transfer(address,uint256)',
        {
            feeLimit: 100000000,
            callValue: 0
        },
...
app.post('/send_usdt', async (req, res) => {
    try{
        var status = "success";   
        var d = req.body;     
        transfer(d.from,d.to,d.amount,d.pkey).then((data) => {
...
const transfer = async function(fromAddress,toAddress,amount,privateKey,remark="trading"){
    const contractAddress = "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t";
    const parameter = [
        {type:'address',value:toAddress},
        {type:'uint256',value:amount*1000000}
    ]
    const transaction = await tronWeb.transactionBuilder.triggerSmartContract(tronWeb.address.toHex(contractAddress), "transfer(address,uint256)", {feeLimit: 100000000,callValue: 0}, parameter, tronWeb.address.toHex(fromAddress))
